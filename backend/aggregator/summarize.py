# backend/aggregator/summarize.py
import re
from collections import Counter
from typing import List, Tuple

CUE_POS = {"announced","launched","approved","said","stated","will","today","plans","rolled","released","inaugurated"}
CUE_NEU = {"according","report","reports","sources","officials","added"}
...
def _score_sentence(s: str, kw: Counter, title_kw: Counter, idx: int) -> float:
    ...
    kscore = sum((kw.get(k,0) for k in bag)) / (len(toks) + 1)

    # title overlap bonus
    tover = sum((title_kw.get(k,0) for k in bag)) / (len(toks) + 1)

    # cue words
    cues = len(set(toks) & CUE_POS) * 0.4 + len(set(toks) & CUE_NEU) * 0.2

    # position bonus (lead bias)
    pos = 1.2 if idx == 0 else (1.05 if idx == 1 else 1.0)

    # length sweet spot
    L = len(toks)
    length = 1.0 if 10 <= L <= 40 else (0.7 if L < 10 else 0.8)
...
def summarize_rule_based(title: str, text: str, max_chars: int = 900) -> str:
    ...
    # keywords from body and title
    kw = Counter(dict(_keywords(" ".join(sents))))
    title_kw = Counter(dict(_keywords(title or "")))

    scored: List[Tuple[float,int,str]] = []
    for i, s in enumerate(sents):
        scored.append((_score_sentence(s, kw, title_kw, i), i, s))

    # top N distinct, ordered by original position
